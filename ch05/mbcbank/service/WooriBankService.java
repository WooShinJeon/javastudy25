...
public class WooriBankService {
	...
	public void menu(Scanner inputStr, Scanner inputInt, AccountDTO[] hanaBank, AccountDTO[] wooriBank, AccountDTO[] nhBank) {
		...
		while(subRun) {
			...
			switch(select) {
			case "1" :
				create(inputStr, inputInt, wooriBank);
				break;
			...
			case "3" :
				deposit(wooriBank, inputStr, inputInt);
				break;
				
			case "4" :
				withdraw(wooriBank, inputStr, inputInt);
				break;
			...
			case "6" :
				System.out.println("우리은행 금융 프로그램을 종료하고 메인메뉴로 돌아갑니다.");
				subRun = false;
				break;
			}
		}
	...
	private void transfer(AccountDTO[] hanaBank, AccountDTO[] wooriBank, AccountDTO[] nhBank, Scanner inputInt, Scanner inputStr) {
		...
		System.out.println("-----------------------");
		AccountDTO transAccount1 = findAccount(wooriBank, wooriAno);
		...
		if(transBankName.equals("하나은행")) {
			...
			System.out.println("이체할 계좌번호를 입력하세요.");
			String wooriAno1 = inputStr.next();
			System.out.println("-----------------------");
			AccountDTO transAccount3 = findAccount(wooriBank, wooriAno1);
	...
	private void withdraw(AccountDTO[] wooriBank, Scanner inputStr, Scanner inputInt) {
		System.out.println("----------------------------------");
		System.out.println("출금을 진행할 계좌번호를 입력하세요.");
		System.out.print(">>> ");
		String ano = inputStr.next();
		System.out.println("-----------------------");
		AccountDTO inputAccount = findAccount(wooriBank, ano);
	...
	private void deposit(AccountDTO[] wooriBank, Scanner inputStr, Scanner inputInt) {
		System.out.println("----------------------------------");
	...
	private AccountDTO findAccount(AccountDTO[] wooriBank, String ano) {
	...
	private void create(Scanner inputStr, Scanner inputInt, AccountDTO[] wooriBank) {
