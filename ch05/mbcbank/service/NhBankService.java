...
public class NhBankService {
	...
	public void menu(Scanner inputStr, Scanner inputInt, AccountDTO[] hanaBank, AccountDTO[] wooriBank, AccountDTO[] nhBank) {
		...
		while(subRun) {
			...
			switch(select) {
			case "1" :
				create(inputStr, inputInt, nhBank);
				break;
			...
			case "3" :
				deposit(nhBank, inputStr, inputInt);
				break;
				
			case "4" :
				withdraw(nhBank, inputStr, inputInt);
				break;
				
			case "5" :
				transfer(nhBank, wooriBank, hanaBank, inputInt, inputStr);
				break;
				
			case "6" :
				System.out.println("농협은행 금융 프로그램을 종료하고 메인메뉴로 돌아갑니다.");
				subRun = false;
				break;
			}
		}
	...
	private void transfer(AccountDTO[] hanaBank, AccountDTO[] wooriBank, AccountDTO[] nhBank, Scanner inputInt, Scanner inputStr) {
		...
		System.out.println("-----------------------");
		AccountDTO transAccount1 = findAccount(nhBank, nhAno);
		...
		if(transBankName.equals("하나은행")) {
			...
			System.out.println("이체할 계좌번호를 입력하세요.");
			String nhAno1 = inputStr.next();
			System.out.println("-----------------------");
			AccountDTO transAccount4 = findAccount(nhBank, nhAno1);
	...
	private void withdraw(AccountDTO[] nhBank, Scanner inputStr, Scanner inputInt) {
		System.out.println("----------------------------------");
		System.out.println("출금을 진행할 계좌번호를 입력하세요.");
		System.out.print(">>> ");
		String ano = inputStr.next();
		System.out.println("-----------------------");
		AccountDTO inputAccount = findAccount(nhBank, ano);
	...
	private void deposit(AccountDTO[] nhBank, Scanner inputStr, Scanner inputInt) {
		System.out.println("----------------------------------");
	...
	private AccountDTO findAccount(AccountDTO[] nhBank, String ano) {
	...
	private void accountLists(AccountDTO[] nhBank) {
		System.out.println("-----------------------");
	...
	private void create(Scanner inputStr, Scanner inputInt, AccountDTO[] nhBank) {
